...
def cover2Content(data):
    global GPy_Table, GTable, wordList
    GPy_Table = {}
    GTable = []
    ...
    #wordList = sorted(set(wordList), key=wordList.index)  # 排序


def write2File(dictName):
    # 保存结果
    filePath = "./dict/" + dictName
    ...
    newlist = list(set(contentlist+wordList))

    with open(filePath, 'w', encoding='utf8') as fd:
        fd.writelines(newlist)